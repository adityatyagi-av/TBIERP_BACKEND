...
const generateAccessAndRefreshToken = async (managerId) => {
  try {
    ...
    const accessToken = await prisma.manager.SignAccessToken({
      where: { id: manager.id },
    });
    const refreshToken = await prisma.manager.SignRefreshToken({
      where: { id: manager.id },
      data: {},
    });
...
const signupManger = asyncHandler(async (req, res) => {
  ...
  const newManager = await prisma.manager.create({
    data: { username, email, password, managertype },
  });
...
const loginManager = asyncHandler(async (req, res) => {
  ...
  const isPasswordCorrect = await prisma.manager.comparePassword({
    data: { username: manager.username, password },
  });

...
const generateAccessAndRefreshToken = async (adminId) => {
  try {
    ...
    const accessToken = await prisma.admin.SignAccessToken({
      where: { id: admin.id },
    });
    const refreshToken = await prisma.admin.SignRefreshToken({
      where: { id: admin.id },
      data: {},
    });
...
const loginAdmin = asyncHandler(async (req, res) => {
  ...
  const isPasswordCorrect = await prisma.admin.comparePassword({
